...
pub trait UrlInfoRepository {
    ...
    async fn add_url_info(&self, key: &str, long_url: &str, short_url: &str) -> Result<UrlInfo, error::Error>;
}
#[derive(Clone, Debug)]
...
impl UrlInfoRepository for DbUrlInfoRepository{
    ...
    async fn add_url_info(&self, key: &str, long_url: &str, short_url: &str) -> Result<UrlInfo, error::Error> {
        let q = "INSERT INTO TABLE (url_info) VALUES ($1, $2, $3) RETURNING key, long_url, short_url" ;
        match sqlx::query(q)
        .bind(key)
        .bind(long_url)
        .bind(short_url)
